...
class DataLoaderBLIP2Text(DataLoaderWrapper):
    '''
    ...
    def LoadOKVQAData(self, module_config):
        ...
        for data_split, vqa_helper in vqa_helpers.items():
            ...
            self.data.okvqa_data[data_split] = load_cached_data(self.config, '{}_data_preprocessed_txt'.format(data_split))
            if not self.data.okvqa_data[data_split]:
                ...
                save_cached_data(self.config, self.data.okvqa_data[data_split], '{}_data_preprocessed_txt'.format(data_split))
