...
class RagModelInstructBLIP(pl.LightningModule):
    ...
    def forward(self, questions, pixel_values, input_ids: torch.Tensor,
        ...
        if 'force_existence' in self.config.model_config.modules:
            ...
            target_encoding = self.generator_tokenizer(text=selected_answers,
                    padding='longest',
        ...
        if False:
        ...
        else: # Skontaj ovaj deo:
            inputs = self.generator_tokenizer(
                images=batch_images_preprocessed, 
                text=questions, # Questions only
                padding='longest',
                max_length=self.config.data_loader.additional.max_decoder_source_length,
                truncation=True,
            ...
            text_based_vision = self.generator_tokenizer(
                text=generator_inputs.generator_input_text_sequences, # Question + other text
                padding='longest',
                max_length=self.config.data_loader.additional.max_decoder_source_length,
                truncation=True,
                return_tensors="pt").to(device, torch.float32)
